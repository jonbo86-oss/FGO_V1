...
function Logo(){return <div className='leading-none text-left'><div className='text-3xl font-black tracking-tight'>facilitea <span style={{color:AQUA}}>GO</span></div><div className='-mt-1 text-right text-xs font-bold text-zinc-700'>by CaixaBank</div></div>}

export default function CashbackPage({onBack}){
  const hero='https://images.unsplash.com/photo-1551650975-87deedd944c3?auto=format&fit=crop&w=1200&q=85'
  const newsletter='https://images.unsplash.com/photo-1516321497487-e288fb19713f?auto=format&fit=crop&w=1000&q=85'
  return <main className='min-h-screen bg-white text-zinc-950' style={{fontFamily:'Inter,Quicksand,system-ui,sans-serif'}}>
    <header className='sticky top-0 z-40 bg-white/90 backdrop-blur border-b'>
      <div className='max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4'>
        ...
        <button onClick={onBack} className='px-5 py-3 rounded-full border font-black'>← Volver a faciliteaGO</button>
      </div>
    </header>

    <section className='max-w-7xl mx-auto px-4 py-10'>
      <div className='text-sm font-black text-zinc-500 mb-5'>Reembolso</div>
      <div className='rounded-[2rem] overflow-hidden bg-[#F3F0E6] grid lg:grid-cols-[1.05fr_.95fr] gap-8 items-center px-8 md:px-16 py-12'>
        <div>
          <span className='inline-flex px-4 py-2 rounded-full bg-white border text-blue-800 font-black'>Pago flexible + cashback</span>
          <h1 className='mt-6 text-4xl md:text-6xl font-black leading-[1.02]'>Disfruta ahora, paga después y recibe tu cashback.</h1>
          <p className='mt-5 text-xl text-zinc-700 max-w-2xl'>Compra en comercios locales adheridos, paga en 4 plazos con tarjeta CaixaBank y recibe tus ventajas faciliteaGO tras confirmar el pedido.</p>
          <div className='mt-8 flex flex-wrap gap-3'>
            <button className='px-7 py-4 rounded-full text-white font-black' style={{backgroundColor:DARK}}>Conseguir cashback</button>
            <button onClick={onBack} className='px-7 py-4 rounded-full bg-white border font-black'>Ver comercios</button>
          </div>
        </div>
    ...
    <section className='max-w-7xl mx-auto px-4 py-8'>
      <div className='rounded-2xl p-8 md:p-10 text-white shadow-xl grid md:grid-cols-4 gap-8 items-center' style={{backgroundColor:DARK}}>
        <div className='text-3xl font-black md:border-r md:border-white/50 md:pr-8'>En faciliteaGO todo son ventajas</div>
        <div><div className='text-3xl mb-3'>▣</div><b className='text-lg'>Pago en 4 plazos</b><p className='text-white/75 mt-2'>Divide tus compras seleccionadas sin perder beneficios.</p></div>
        <div><div className='text-3xl mb-3'>◎</div><b className='text-lg'>Cashback CaixaBank</b><p className='text-white/75 mt-2'>Acumula reembolso en compras locales adheridas.</p></div>
        <div><div className='text-3xl mb-3'>▤</div><b className='text-lg'>Recogida o envío</b><p className='text-white/75 mt-2'>Elige recogida en tienda o envío a domicilio.</p></div>
      </div>
    </section>

    <section className='max-w-7xl mx-auto px-4 py-12 grid lg:grid-cols-3 gap-6'>
      {[['1','Compra en faciliteaGO','Elige productos de comercios locales adheridos y añade el cupón en el carrito cuando aplique.'],['2','Selecciona pago flexible','En el checkout, selecciona CaixaBank Pay o pago con tarjeta compatible para simular el pago en plazos.'],['3','Recibe el cashback','Tras confirmar el pedido, el reembolso se refleja en tu área Mi cuenta junto con puntos faciliteaGO.']].map(([n,t,d])=><div key={n} className='rounded-[2rem] bg-white border shadow-sm p-7'><div className='w-12 h-12 rounded-2xl text-white grid place-items-center font-black' style={{backgroundColor:n==='2'?AQUA:BLUE}}>{n}</div><h3 className='mt-5 text-2xl font-black'>{t}</h3><p className='mt-3 text-zinc-600'>{d}</p></div>)}
    </section>
    ...
    <section className='max-w-7xl mx-auto px-4 py-12'>
      <div className='rounded-[2rem] p-8 md:p-14 grid lg:grid-cols-[.85fr_1.15fr] gap-10 items-center' style={{backgroundColor:'#346B72'}}>
        <img src={newsletter} className='w-full h-[330px] object-cover rounded-[1.5rem]'/>
